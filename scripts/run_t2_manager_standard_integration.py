#!/usr/bin/env python3
"""
T2 — Validación integración MANAGER_STANDARD sobre tenant NUEVO (sin repair).

Checks:
  1) Onboarding crea tenant (trial)
  2) SQL: rol_permiso MANAGER_TENANT contiene bundle (47)
  3) SQL: rol_menu_permiso MANAGER_TENANT (puede_ver=1) = 14
  4) Crear usuario manager + assign rol vía API
  5) GET /auth/menu con token manager => modulos contiene ORG + INV y no hay permisos.eliminar=true

Evidencia JSON:
  app/bootstrap_v2/00_manifest/evidence/T2_MANAGER_STANDARD_INTEGRATION_VALIDATION.json
"""
...
def _create_tenant(
    ...
    subdominio = f"t2mgr{suffix}"
    codigo = f"T2{suffix.upper()[:5]}"
    payload = {
        "codigo_cliente": codigo,
        "subdominio": subdominio,
        "razon_social": f"T2 MANAGER_STANDARD Tenant {suffix}",
        "nombre_comercial": f"T2MGR {suffix}",
        "ruc": "20987654321",
        "tipo_instalacion": "shared",
        "modo_autenticacion": "local",
        "contacto_email": f"admin+{suffix}@t2mgr.local",
        "contacto_nombre": "T2 Admin",
...
SELECT rol_id
FROM rol
WHERE cliente_id = :cid AND codigo_rol = 'MANAGER_TENANT' AND es_activo = 1
"""
...
async def _sql_snapshot(cliente_id: UUID, *, label: str) -> Dict[str, Any]:
    cid = str(cliente_id)
    codes = list(MANAGER_STANDARD_PERMISSION_CODIGOS)
    ...
    async with get_db_connection(DatabaseConnection.ADMIN) as session:
        row = (await session.execute(text(SQL_MANAGER_ROLE).bindparams(cid=cid))).fetchone()
        rid = row[0] if row else None
        if rid is None:
            return {"label": label, "ok": False, "error": "MANAGER_TENANT rol no encontrado"}
        rid_str = str(rid)
        rp = (
    ...
    return {
        "label": label,
        "sql_queries": {
            "manager_role": SQL_MANAGER_ROLE.strip(),
        ...
        "expected": {
            "rol_permiso_bundle_count": len(MANAGER_STANDARD_PERMISSION_CODIGOS),
            "rol_menu_permiso_ver_count": len(MANAGER_STANDARD_MENU_GRANTS),
        },
        "checks": {
            "rp_ok": rp_count >= len(MANAGER_STANDARD_PERMISSION_CODIGOS),
            "rmp_ok": rmp_count >= len(MANAGER_STANDARD_MENU_GRANTS),
        },
    }


def _manager_menu_check(menu_body: Any) -> Dict[str, Any]:
    ...
    for m in modulos:
        ...
        for s in m.get("secciones") or []:
            ...
            for menu in s.get("menus") or []:
                ...
                if isinstance(perms, dict) and bool(perms.get("eliminar")):
                    any_delete_true = True
    return {
        "ok": True,
        "modulo_codigos": codigos,
        "has_org": has_org,
        "has_inv": has_inv,
        "any_eliminar_true": any_delete_true,
    }
...
async def run(
    *, base_url: str, fe_port: str, platform_user: str, platform_password: str
    ...
    report: Dict[str, Any] = {
        "report": "T2_MANAGER_STANDARD_INTEGRATION_VALIDATION",
        "timestamp_utc": _utc_now(),
        "base_url": base_url,
        "repair_used": False,
        "expected_permiso_codigos_count": len(MANAGER_STANDARD_PERMISSION_CODIGOS),
        "expected_menu_ver_count": len(MANAGER_STANDARD_MENU_GRANTS),
        "steps": {},
    }
    ...
    report["steps"]["manager_create"] = {
        "status": create_user.status,
        "ok": create_user.status in (200, 201),
    }
    if create_user.status not in (200, 201) or not isinstance(create_user.body, dict):
        report["verdict"] = {
            "status": "FAIL",
            "reason": "manager_create_failed",
            "body": create_user.body,
        }
        return report

    usuario_id = (create_user.body.get("data") or {}).get("usuario_id") or create_user.body.get(
        "usuario_id"
    )
    report["steps"]["manager_create"]["usuario_id"] = usuario_id

    async with get_db_connection(DatabaseConnection.ADMIN) as session:
        row = (
            await session.execute(text(SQL_MANAGER_ROLE).bindparams(cid=str(cid)))
        ).fetchone()
        manager_rol_id = str(row[0]) if row else None
    if not manager_rol_id:
        report["verdict"] = {"status": "FAIL", "reason": "manager_role_missing"}
        return report

    assign = client.request(
        "POST",
        f"/api/v1/usuarios/{usuario_id}/roles/{manager_rol_id}/",
        headers={"Origin": tenant_origin(sub, fe_port), "X-Client-Type": "mobile"},
        bearer=admin_token,
        json_body={},
    )
    report["steps"]["manager_assign"] = {"status": assign.status, "ok": assign.status in (200, 201)}
    ...
    report["steps"]["manager_login"] = {"passed": bool(manager_token)}
    if not manager_token:
        report["verdict"] = {"status": "FAIL", "reason": "manager_login_failed"}
        return report
    ...
    check = _manager_menu_check(menu.body)
    report["steps"]["manager_menu"] = {"status": menu.status, "check": check}
    ...
    report["verdict"] = {
        ...
        "no_delete": not bool(check.get("any_eliminar_true")),
    }
    return report


def main() -> int:
    parser = argparse.ArgumentParser(description="T2 MANAGER_STANDARD integration")
