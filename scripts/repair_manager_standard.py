#!/usr/bin/env python3
"""
T2 — Aplicar bundle MANAGER_STANDARD a MANAGER_TENANT.

Idempotente.

Uso:
  python scripts/repair_manager_standard.py --dry-run --all
  python scripts/repair_manager_standard.py --subdominio prueba --apply
  python scripts/repair_manager_standard.py --cliente-id <UUID> --apply
"""
    ...
    rows = (
        await session.execute(
            text(
                ...
                WHERE c.es_activo = 1
                  AND r.codigo_rol = 'MANAGER_TENANT'
                  AND r.es_activo = 1
...
async def _audit_one(session, *, cliente_id: UUID) -> Dict[str, Any]:
    rid = await ManagerStandardService.resolve_manager_rol_id(session, cliente_id=cliente_id)
    if not rid:
        return {
            ...
            "needs_repair": True,
            "reason": "MANAGER_ROLE_NOT_FOUND",
        }

    ph = ", ".join(f":pc{i}" for i in range(len(MANAGER_STANDARD_PERMISSION_CODIGOS)))
    bind = {f"pc{i}": MANAGER_STANDARD_PERMISSION_CODIGOS[i] for i in range(len(MANAGER_STANDARD_PERMISSION_CODIGOS))}
    sql_rp = text(
        f"""
    ...
    expected_rp = len(MANAGER_STANDARD_PERMISSION_CODIGOS)
    expected_rmp = len(MANAGER_STANDARD_MENU_GRANTS)
...
async def _run(
    *,
    ...
    async with get_db_connection(DatabaseConnection.ADMIN) as session:
        ...
        if apply:
            for cid in tenant_ids:
                rid = await ManagerStandardService.resolve_manager_rol_id(
                    session, cliente_id=cid
                )
                if not rid:
                    continue
                one = await ManagerStandardService.apply_to_manager_role(
                    session, cliente_id=cid, manager_rol_id=rid
                )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="T2 MANAGER_STANDARD repair")
