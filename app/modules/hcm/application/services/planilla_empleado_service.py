...
async def create_planilla_empleado(
    ...
    await ensure_planilla_borrador(client_id, data.planilla_id)
...
async def update_planilla_empleado(
    ...
    current = await _get(client_id, planilla_empleado_id)
    if not current:
        raise NotFoundError(f"Planilla empleado {planilla_empleado_id} no encontrado")
    await ensure_planilla_borrador(client_id, current["planilla_id"])
