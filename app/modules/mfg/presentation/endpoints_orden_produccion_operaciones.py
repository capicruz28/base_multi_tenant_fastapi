...
@router.get(
    "",
    response_model=List[OrdenProduccionOperacionRead],
    tags=["MFG - OP Operaciones"],
    dependencies=[Depends(require_permission("mfg.orden_produccion_operacion.leer"))],
)
...
@router.get(
    "/{op_operacion_id}",
    response_model=OrdenProduccionOperacionRead,
    tags=["MFG - OP Operaciones"],
    dependencies=[Depends(require_permission("mfg.orden_produccion_operacion.leer"))],
)
...
@router.post(
    ...
    tags=["MFG - OP Operaciones"],
    dependencies=[Depends(require_permission("mfg.orden_produccion_operacion.crear"))],
)
...
@router.put(
    ...
    response_model=OrdenProduccionOperacionRead,
    tags=["MFG - OP Operaciones"],
    dependencies=[Depends(require_permission("mfg.orden_produccion_operacion.actualizar"))],
)
