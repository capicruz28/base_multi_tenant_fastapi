...
async def get_zona_almacen_by_id(client_id: UUID, empresa_id: UUID, zona_id: UUID) -> ZonaAlmacenRead:
    """Obtiene una zona por id. Lanza NotFoundError si no existe."""
    row = await _get_zona_almacen_by_id(client_id, empresa_id, zona_id)
    if not row:
...
async def create_zona_almacen(client_id: UUID, data: ZonaAlmacenCreate) -> ZonaAlmacenRead:
    ...
    row = await _create_zona_almacen(
        client_id,
        data.empresa_id,
...
async def update_zona_almacen(
    client_id: UUID, empresa_id: UUID, zona_id: UUID, data: ZonaAlmacenUpdate
) -> ZonaAlmacenRead:
