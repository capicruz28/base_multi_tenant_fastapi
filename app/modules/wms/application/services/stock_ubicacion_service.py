...
async def get_stock_ubicacion_by_id(client_id: UUID, empresa_id: UUID, stock_ubicacion_id: UUID) -> StockUbicacionRead:
    """Obtiene un stock por ubicación por id. Lanza NotFoundError si no existe."""
    row = await _get_stock_ubicacion_by_id(client_id, empresa_id, stock_ubicacion_id)
    if not row:
...
async def create_stock_ubicacion(client_id: UUID, data: StockUbicacionCreate) -> StockUbicacionRead:
    ...
    row = await _create_stock_ubicacion(
        client_id,
        data.empresa_id,
...
async def update_stock_ubicacion(
    client_id: UUID, empresa_id: UUID, stock_ubicacion_id: UUID, data: StockUbicacionUpdate
) -> StockUbicacionRead:
