...
CREATE TABLE modulo (
    ...
    -- CRÍTICO: Usar en enums del backend
    ...
    -- El módulo LOGISTICA podría requerir ALMACEN activo
    ...
    -- Ejemplo: '{"periodo_fiscal": "enero-diciembre", "usa_series": true}'
);
...
-- Relación: cliente 1:N cliente_modulo
-- ============================================================================
CREATE TABLE cliente_modulo (
    ...
    -- Control on/off del módulo para este cliente
    
    fecha_activacion DATETIME DEFAULT GETDATE(),
    fecha_vencimiento DATETIME NULL,
    -- NULL = licencia ilimitada/permanente
    -- Con valor = fecha de expiración de la licencia
    
    modo_prueba BIT DEFAULT 0,
    -- TRUE = Cliente en modo trial del módulo
    fecha_fin_prueba DATETIME NULL,
    -- Fecha límite del periodo de prueba
    
    -- ======================================== 
    -- CONFIGURACIÓN ESPECÍFICA DEL CLIENTE
    -- ======================================== 
    configuracion_json NVARCHAR(MAX) NULL,
    -- Configuración personalizada del módulo para este cliente
    -- Sobreescribe configuracion_defecto del módulo
    -- Ejemplo: '{"moneda_principal": "PEN", "usa_multipuntos": true}'
    
    -- ======================================== 
    -- LÍMITES Y CUOTAS (si aplica)
    -- ======================================== 
    limite_usuarios INT NULL,
    -- Máximo de usuarios con acceso a este módulo
    -- NULL = ilimitado
    
    limite_registros INT NULL,
    -- Límite de registros principales (ej: productos, empleados)
    -- NULL = ilimitado
    ...
    -- CONSTRAINTS
    -- ======================================== 
    CONSTRAINT FK_cliente_modulo_cliente 
        FOREIGN KEY (cliente_id) REFERENCES cliente(cliente_id) 
        ON DELETE CASCADE,
    CONSTRAINT FK_cliente_modulo_modulo 
...
CREATE TABLE modulo_seccion (
    ...
    modulo_id UNIQUEIDENTIFIER NOT NULL,
    -- A qué módulo pertenece esta sección
    ...
    -- Ejemplos: 'RUTAS', 'VEHICULOS', 'CONDUCTORES'
    
    nombre NVARCHAR(100) NOT NULL,
    -- Nombre descriptivo
    -- Ejemplos: 'Gestión de Rutas', 'Control de Vehículos'
    ...
    -- TRUE = Sección predefinida (no editable por cliente)
    -- FALSE = Sección personalizada creada por cliente
...
CREATE TABLE modulo_menu (
    ...
    -- Sección dentro del módulo (puede ser NULL para menús sin sección)
    
    cliente_id UNIQUEIDENTIFIER NULL,
    -- NULL = Menú global del sistema (todos los clientes lo ven)
    -- Con valor = Menú personalizado creado por un cliente específico
    
    -- ======================================== 
    -- IDENTIFICACIÓN
    -- ======================================== 
    codigo NVARCHAR(50) NULL,
    -- Código único para referencia en código (solo menús del sistema)
    -- Ejemplos: 'RUTAS_LISTAR', 'VEHICULOS_CREAR', 'RUTAS_REPORTES'
    
    nombre NVARCHAR(100) NOT NULL,
    -- Nombre para mostrar en UI
    -- Ejemplos: 'Listar Rutas', 'Nueva Ruta', 'Reportes de Rutas'
    ...
    -- Ejemplos: '/logistica/rutas', '/logistica/rutas/nueva'
    ...
    -- NULL = menú raíz
    -- Con valor = este es un submenú
    
    nivel INT DEFAULT 1,
    -- Nivel de profundidad: 1=raíz, 2=hijo, 3=nieto
    -- Útil para limitar niveles de anidación
    
    -- ======================================== 
    -- CONFIGURACIÓN
    -- ======================================== 
    tipo_menu NVARCHAR(20) DEFAULT 'pantalla',
    -- 'pantalla' = Abre una vista/componente
    -- 'accion' = Ejecuta una acción (ej: exportar)
    -- 'enlace_externo' = Abre URL externa
    ...
    -- FALSE = Ruta accesible pero no se muestra en menú
    
    es_menu_sistema BIT DEFAULT 1,
    -- TRUE = Menú del sistema (no editable por cliente)
    -- FALSE = Menú personalizado del cliente
    ...
    -- Ejemplo: '{"abre_modal": true, "requiere_confirmacion": true}'
    ...
    CONSTRAINT FK_menu_modulo 
        FOREIGN KEY (modulo_id) REFERENCES modulo(modulo_id) 
        ON DELETE CASCADE,
    CONSTRAINT FK_menu_seccion 
        FOREIGN KEY (seccion_id) REFERENCES modulo_seccion(seccion_id) 
        ON DELETE NO ACTION,
    CONSTRAINT FK_menu_cliente 
        FOREIGN KEY (cliente_id) REFERENCES cliente(cliente_id) 
        ON DELETE CASCADE,
    CONSTRAINT FK_menu_padre 
        FOREIGN KEY (menu_padre_id) REFERENCES modulo_menu(menu_id) 
        ON DELETE NO ACTION
);
...
-- Propósito: Permisos granulares de roles sobre menús (SIN CAMBIOS MAYORES)
-- Solo se actualiza FK para apuntar a modulo_menu
-- ============================================================================
CREATE TABLE rol_menu_permiso (
    ...
    -- Agregar más según necesidad del negocio
    
    -- ======================================== 
    -- PERMISOS PERSONALIZADOS POR MÓDULO
    -- ======================================== 
    permisos_extra NVARCHAR(MAX) NULL,
    -- JSON con permisos específicos del módulo/pantalla
    -- Ejemplo Logística: '{"puede_cerrar_ruta": true, "puede_reasignar_conductor": false}'
    -- Ejemplo Almacén: '{"puede_ajustar_inventario": true, "puede_cerrar_periodo": false}'
    ...
    CONSTRAINT FK_permiso_menu 
        FOREIGN KEY (menu_id) REFERENCES modulo_menu(menu_id) 
        ON DELETE NO ACTION,
    CONSTRAINT FK_permiso_cliente 
        FOREIGN KEY (cliente_id) REFERENCES cliente(cliente_id) 
        ON DELETE NO ACTION,
...
CREATE TABLE modulo_rol_plantilla (
    ...
    -- Ejemplos: 'Administrador Logística', 'Operador Almacén', 'Consulta Ventas'
    
    descripcion NVARCHAR(255) NULL,
    
    nivel_acceso INT DEFAULT 1,
    -- 1=Básico, 2=Intermedio, 3=Avanzado, 4=Administrador
    
    -- ======================================== 
    -- PERMISOS POR DEFECTO
    -- ======================================== 
    permisos_json NVARCHAR(MAX) NULL,
    -- JSON con estructura de permisos por menú
    -- Ejemplo:
    -- {
    --   "RUTAS_LISTAR": {"ver": 1, "crear": 1, "editar": 1, "eliminar": 0},
    --   "VEHICULOS_LISTAR": {"ver": 1, "crear": 0, "editar": 0, "eliminar": 0}
    -- }
    ...
    CONSTRAINT FK_plantilla_modulo 
        FOREIGN KEY (modulo_id) REFERENCES modulo(modulo_id) 
        ON DELETE CASCADE
);
