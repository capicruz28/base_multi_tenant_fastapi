...
async def _ensure_empresa_belongs_to_tenant(client_id: UUID, empresa_id: UUID) -> None:
    """
    Valida que (cliente_id, empresa_id) exista en org_empresa.
    Evita cruces de empresa dentro del mismo cliente y previene inserts inválidos.
    """
...
async def list_zonas_almacen(
    ...
    query = select(WmsZonaAlmacenTable).where(
        and_(
            ...
            WmsZonaAlmacenTable.c.empresa_id == empresa_id,
        )
    )
    if almacen_id:
...
async def get_zona_almacen_by_id(client_id: UUID, empresa_id: UUID, zona_id: UUID) -> Optional[Dict[str, Any]]:
...
async def create_zona_almacen(client_id: UUID, empresa_id: UUID, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await get_zona_almacen_by_id(client_id, empresa_id, payload["zona_id"])


async def update_zona_almacen(
    client_id: UUID, empresa_id: UUID, zona_id: UUID, data: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
    """Actualiza una zona de almacén. WHERE incluye cliente_id y zona_id."""
    await _ensure_empresa_belongs_to_tenant(client_id=client_id, empresa_id=empresa_id)
    payload = {
        k: v for k, v in data.items()
        if k in _COLUMNS and k not in ("zona_id", "cliente_id", "empresa_id")
    }
    if not payload:
        return await get_zona_almacen_by_id(client_id, empresa_id, zona_id)
    stmt = (
