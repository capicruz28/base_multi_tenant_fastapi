...
async def list_stock_alertas_bajo_minimo(
    ...
    """
    Lista stocks con nivel bajo mínimo.
    Calcula disponible como: cantidad_actual - cantidad_reservada.

    Nota: se evita depender de columnas calculadas (pueden no estar en metadata).
    """
    query = select(InvStockTable).where(InvStockTable.c.cliente_id == client_id)
    if empresa_id:
        query = query.where(InvStockTable.c.empresa_id == empresa_id)
    if almacen_id:
        query = query.where(InvStockTable.c.almacen_id == almacen_id)
