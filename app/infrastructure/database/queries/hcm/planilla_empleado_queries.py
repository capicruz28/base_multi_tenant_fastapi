...
from app.infrastructure.database.tables_erp import HcmPlanillaTable, HcmPlanillaEmpleadoTable
...
async def create_planilla_empleado(client_id: UUID, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fase 5: empresa_id es obligatorio; derivarlo desde la planilla (cabecera)
    planilla_id = payload.get("planilla_id")
    if planilla_id:
        q = select(HcmPlanillaTable.c.empresa_id).where(
            and_(
