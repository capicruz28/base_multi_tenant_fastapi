...
def get_connection_metadata(client_id: int) -> Dict[str, Any]:
    ...
    ✅ FASE 2: Cache mejorado con Redis
    ...
    if cached_metadata:
        logger.debug(f"[METADATA] Cache memoria HIT para cliente {client_id}")
    ...
    try:
        ...
        if metadata:
            # ✅ FASE 2: Guardar en ambos caches (Redis y memoria)
            connection_cache.set(client_id, metadata)
            
            # Guardar también en Redis (si está habilitado)
            if settings.ENABLE_REDIS_CACHE:
                try:
                    from app.infrastructure.cache.redis_cache import set_cached
                    set_cached(f"connection_metadata:{client_id}", metadata, ttl=600)  # 10 minutos
                except Exception as e:
                    logger.debug(f"[METADATA] Error guardando en Redis (no crítico): {e}")
            ...
            # ✅ FASE 2: Cachear fallback en ambos caches
            connection_cache.set(client_id, fallback_metadata)
            
            if settings.ENABLE_REDIS_CACHE:
                try:
                    from app.infrastructure.cache.redis_cache import set_cached
                    set_cached(f"connection_metadata:{client_id}", fallback_metadata, ttl=600)
                except Exception:
                    pass  # No crítico
